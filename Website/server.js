...
app.get("/button-color", async (req, res) => {
  try {
    const jsonDataArray = await fs.readFile("data.json", "utf8");
...
app.post("/changed-json", async (req, res) => {
...
app.route("/logout").post(dashboardController.logout);
...
app.route("/updatePassword").post(updatePasswordController.updatePassword);
